%变速又变调
function output=Process(handles,freaq_rate,rate)
xx=handles.y;
fs=handles.Fs;
...
[Dpitch,Dfreq,Ef,SF,voiceseg,vosl,vseg,vsl,T2]=...
    Ext_F0ztms(xx,fs,wlen,inc,T1,r2,miniL,mnlong,ThrC);
...
axes(handles.huitu3);
plot(frameTime,Dpitch,'k');
hold on
line(frameTime,Dpitchm2,'color',[.6 .6 .6]);
axis([0 max(time) 0 120]);title('基音周期'); ylabel('样点值');
hold off
axes(handles.huitu4);
plot(frameTime,Dfreq,'k'); 
hold on
...
axis([0 max(time) 0 450]);title('基音频率'); ylabel('频率/Hz');
xlabel('时间/s'); 
legend('原基音频率','改变的基音频率');
hold off

axes(handles.huitu);
plot(time,x); title('原始语音波形'); 
axis([0 max(time) -1 1]); xlabel('时间/s'); ylabel('幅值')
axes(handles.huitu2);
plot(time1,output);  title('合成语音波形');
